...
public class test {
	 ...
	 * ann.setTitle(" 개인정보 처리방침 변경 안내('22.07.07)"); ann.setU_id("wkdwndus0409"); //
	 * ann.setWrdate(sqlDate);
	 * ann.setMain_text("현대그린푸드 홈페이지를 방문하여 주신 이용자께 감사의 말씀을 드리며,\r\n" +
	 * "현대그린푸드의 개인정보처리방침을 개정함에 따라 그 변경 내용을 알려드립니다."); ann.setfile_id(0); //
	 * ann.setVisiter(34); ann.setAnnoun_type(1);
	 * 
	 * AnnouncementDAO annDAO = AnnouncementDAO.getInstance(); annDAO.write(ann);
	 * 
	 ...
	 * 
	 * for (int i = 0; i < annList.size(); i++) { AnnouncementVO ann =
	 * annList.get(i); System.out.println(ann.getId());
	 * System.out.println(ann.getU_id()); System.out.println(ann.getTitle());
	 * System.out.println(ann.getMain_text()); System.out.println(ann.getWrdate());
	 * }
	 * 
	 ...
	 * 
	 * AnnouncementVO ann = new AnnouncementVO(); ann.setId(7);
	 * ann.setTitle("현대그린푸드, 국내 첫 '블루존 건강식단' 선보인다"); ann.setMain_text(
	 ...
	 * ); ann.setfile_id(0); ann.setAnnoun_type(0);
	 * 
	 * AnnouncementDAO annDAO = AnnouncementDAO.getInstance(); annDAO.update(ann);
	 * 
	 * System.out.println("공지사항_수정 success"); }
	 * 
	 * public static void 공지사항_삭제() {
	 * 
	 * AnnouncementDAO annDAO = AnnouncementDAO.getInstance(); annDAO.delete(11);
	 * 
	 ...
	 * 
	 * AnnouncementDAO annDAO = AnnouncementDAO.getInstance();
	 * ArrayList<AnnouncementVO> annList = annDAO.get_ALL_List("개인"); for (int i =
	 * 0; i < annList.size(); i++) { AnnouncementVO ann = annList.get(i);
	 ...
	 * annList.get(i); System.out.println(ann.getId());
	 * System.out.println(ann.getU_id()); System.out.println(ann.getTitle());
	 * System.out.println(ann.getVisiter()); System.out.println(ann.getWrdate());
...
public static void cnt확인() {
	 ...
	 * 
	 * AnnouncementDAO annDAO = AnnouncementDAO.getInstance(); AnnouncementVO ann =
	 * annDAO.getAnn(56);
	 * 
	 * System.out.println(ann.getId()); System.out.println(ann.getTitle());
	 * System.out.println(ann.getU_id()); System.out.println(ann.getWrdate());
	 * //null System.out.println(ann.getMain_text()); //null
	 * System.out.println(ann.getfile_id()); System.out.println(ann.getVisiter());
	 * System.out.println(ann.getAnnoun_type()); }
	 */
