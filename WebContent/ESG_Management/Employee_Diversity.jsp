<!--작성자 김찬중-->
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" trimDirectiveWhitespaces="true"%>
<!DOCTYPE html>
<html lang="en">

<head>
	<meta charset="UTF-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Mangerial_Objective</title>
	<link href="../css/EM_style.css" rel="stylesheet" type="text/css">
</head>

<body>
	<%@ include file="/Inc/Header.jspf"%>
	<div class="container_area" id="contents" <div class="sub_layout esg_area">
		<div class="sub_visual">
			<ul class="hierarchy_list">
				<li class="icon_home"><img src="../img/icon/icon_home.png" alt="Home" /></li>
				<li class="depth_2">
					<a href="#" class="m_hierarchy">ESG 경영 </a>
					<ul class="hierarchy_depth" style="display: none;">
						<li><a href="#">사회공헌(S)</a></li>
					</ul>
				</li>
				<li class="depth_3"><a href="#" class="t_block_hierarchy">사회공헌(S)</a>
					<ul class="hierarchy_depth">
						<li><a href="#">경영목표</a></li>
						<li><a href="#">환경경영(E)</a></li>
						<li class="on"><a href="#">사회공헌(S)</a></li>
						<li><a href="#">지배구조(G)</a></li>
						<li><a href="#">안전보건</a></li>
						<li><a href="#">상생경영</a></li>
						<li><a href="#">윤리경영</a></li>
					</ul>
				</li>
			</ul>
			<div class="title_description">
				<h2 class="title_sub">ESG 경영</h2>
				<p class="sub_description">
					현대그린푸드는 ‘건강’과 ‘지속가능성’ 이라는 비전을 위해
					<br>다양한 분야에서 사회적 가치를 창출하는 것을 목표로 합니다.
				</p>
			</div>
		</div>

		<div class="category">
			<ul>
				<li><a href="Managerial_Objective.jsp">경영목표</a> </li>
				<li><a href="NetZero_2030.jsp">환경경영(E)</a></li>
				<li class="on"><a href="CRS.jsp">사회공헌(S)</a> </li>
				<li><a href="Board_Policy.jsp">지배구조(G)</a> </li>
				<li><a href="Safety_Health.jsp">안전보건</a> </li>
				<li><a href="Win_Management.jsp">상생경영</a> </li>
				<li><a href="Group_Ethical_Management.jsp">윤리경영</a> </li>
			</ul>
		</div>
		<div class="sub_contents">
			<ul class="tab_list">
				<li><a href="CRS.jsp">CSR</a></li>
				<li class="on"><a href="#;">임직원 다양성</a></li>
			</ul>

			<section class="esg_section cont3_2_1">
				<h3 class="esg_title">임직원 다양성</h3>
				<div class="esg_text">
					현대그린푸드는 포괄적인 인재채용을 통해
					<br class="pc">임직원의 다양성을 존중하는 문화 구축에 앞장섭니다.
				</div>

				<div class="director_area">
					<div>
						<table class="list_tb">
							<colgroup>
								<col width="20%" />
								<col width="20%" />
								<col width="10%" />
								<col width="15%" />
								<col width="15%" />
								<col width="20%" />
							</colgroup>
							<thead>
								<tr>
									<th scope="col" class="first" colspan="2">구분</th>
									<th scope="col">단위</th>
									<th scope="col">2020년</th>
									<th scope="col">2021년</th>
									<th scope="col">비고</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td class="first" rowspan="2">자발적 이직률</td>
									<td>이직 인원 계</td>
									<td>명</td>
									<td>267</td>
									<td>401</td>
									<td rowspan="32">
										<ol class="director_division_list">
											<li>※ <strong>관리자</strong></li>
											<li>
												기업의 공식 조직도 상
												<br>직급과는 무관하며
												<br>각급 부서/영업점 단위
												<br>책임자를 말함.
												<br>(업무지휘 감독권,
												<br>인사고과권, 결재권 보유자)
											</li>
										</ol>
									</td>
								</tr>
								<tr>
									<td>자발적 이직</td>
									<td>명, %</td>
									<td class="right_td">247 (92.5%)</td>
									<td class="right_td">378(94.3%)</td>
								</tr>
								<tr>
									<td class="first" rowspan="3">성별 인력현황</td>
									<td>계</td>
									<td>명</td>
									<td>5397</td>
									<td>5881</td>
								</tr>
								<tr>
									<td>남성</td>
									<td>명, %</td>
									<td>1774 (32.9%)</td>
									<td>2044(34.8%)</td>
								</tr>
								<tr>
									<td>여성</td>
									<td>명, %</td>
									<td>3623 (67.1%)</td>
									<td>3837(65.2%)</td>
								</tr>
								<tr>
									<td class="first" rowspan="6">성별 임원/ <br>관리자 현황</td>
									<td>임원 계</td>
									<td>명</td>
									<td>12</td>
									<td>12</td>
								</tr>
								<tr>
									<td>남성</td>
									<td>명, %</td>
									<td>11(91.7%)</td>
									<td>11(91.7%)</td>
								</tr>
								<tr>
									<td>여성</td>
									<td>명, %</td>
									<td>1(8.3%)</td>
									<td>1(8.3%)</td>
								</tr>
								<tr>
									<td>관리자 계</td>
									<td>명</td>
									<td>456</td>
									<td>447</td>
								</tr>
								<tr>
									<td>남성</td>
									<td>명, %</td>
									<td>187(41.0%)</td>
									<td>176(39.4%)</td>
								</tr>
								<tr>
									<td>여성</td>
									<td>명, %</td>
									<td>269(59.0%)</td>
									<td>271(60.6%)</td>
								</tr>
								<tr>
									<td class="first" rowspan="6">고용형태 <br>(정규직, 기간제) </td>
									<td>정규직 계</td>
									<td>명</td>
									<td>5072</td>
									<td>5881</td>
								</tr>
								<tr>
									<td>남성</td>
									<td>명, %</td>
									<td>1678(33.1%)</td>
									<td>2019(34.3%)</td>
								</tr>
								<tr>
									<td>여성</td>
									<td>명, %</td>
									<td>3394(66.9%)</td>
									<td>3523(59.9%)</td>
								</tr>
								<tr>
									<td>기간제 계</td>
									<td>명</td>
									<td>325</td>
									<td>339</td>
								</tr>
								<tr>
									<td>남성</td>
									<td>명, %</td>
									<td>49(15.1%)</td>
									<td>23(6.8%)</td>
								</tr>
								<tr>
									<td>여성</td>
									<td>명, %</td>
									<td>276(84.9%)</td>
									<td>316(93.2%)</td>
								</tr>
								<tr>
									<td class="first" rowspan="5">연령별 인력현황</td>
									<td>계</td>
									<td>명</td>
									<td>5397</td>
									<td>5881</td>
								</tr>
								<tr>
									<td>20대</td>
									<td>명, %</td>
									<td>1175(21.9%)</td>
									<td>1199(20.4%)</td>
								</tr>
								<tr>
									<td>30대</td>
									<td>명, %</td>
									<td>1292(24.1%)</td>
									<td>1372(23.3%)</td>
								</tr>
								<tr>
									<td>40대</td>
									<td>명, %</td>
									<td>1232(23.0%)</td>
									<td>1140(19.4%)</td>
								</tr>
								<tr>
									<td>50대 이상</td>
									<td>명, %</td>
									<td>1669(31.1%)</td>
									<td>2170(37.0%)</td>
								</tr>
								<tr>
									<td class="first" rowspan="4">학력별 인력현황</td>
									<td>계</td>
									<td>명</td>
									<td>5397</td>
									<td>5881</td>
								</tr>
								<tr>
									<td>석사이상</td>
									<td>명, %</td>
									<td>80(1.5%)</td>
									<td>86(1.5%)</td>
								</tr>
								<tr>
									<td>대졸</td>
									<td>명, %</td>
									<td>1227(22.7%)</td>
									<td>1168(19.9%)</td>
								</tr>
								<tr>
									<td>초대졸 이하</td>
									<td>명, %</td>
									<td>4090(75.8%)</td>
									<td>4627(78.7%)</td>
								</tr>
								<tr>
									<td class="first" rowspan="4">직군별 인력현황</td>
									<td>계</td>
									<td>명</td>
									<td>5397</td>
									<td>5881</td>
								</tr>
								<tr>
									<td>임원</td>
									<td>명, %</td>
									<td>12(0.2%)</td>
									<td>12(0.2%)</td>
								</tr>
								<tr>
									<td>사무직</td>
									<td>명, %</td>
									<td>377(7.0%)</td>
									<td>400(6.8%)</td>
								</tr>
								<tr>
									<td>전문·생산·기술직</td>
									<td>명, %</td>
									<td>5008(92.8%)</td>
									<td>5469(93.0%)</td>
								</tr>
								<tr>
									<td class="first" rowspan="2">장애인 고용현황</td>
									<td>장애인 근로자 수</td>
									<td>명</td>
									<td>108</td>
									<td>132</td>
								</tr>
								<tr>
									<td>고용률</td>
									<td>%</td>
									<td>3.23%</td>
									<td>4.03%</td>
								</tr>
							</tbody>
						</table>
						<p class="korean_financial" style="text-align:left">※ 여성 임직원 비율 60% 이상 목표</p>
						<p class="korean_financial" style="text-align:left">※ 장애인 고용 비율 3.1% 이상 목표</p>
					</div>
				</div>
			</section>

			<section class="esg_section cont3_2_2">
				<h3 class="esg_title">임직원 교육</h3>
				<div class="esg_text">
					현대그린푸드는 식품산업의 미래를 주도적으로 이끌어 나갈 인재를 양성하기 위해
					<br class="pc">다양한 성장지원 프로그램을 운영하고 있습니다.
				</div>

				<div class="director_area">
					<div>
						<table class="list_tb">
							<colgroup>
								<col width="20%" />
								<col width="" />
							</colgroup>
							<thead>
								<tr>
									<th scope="col" class="first">교육과정명</th>
									<th scope="col">세부내용</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td class="first">
										신입/경력사원
										<br>온보딩 프로그램
									</td>
									<td>· 회사소개, 회사생활 가이드, 직무기초, 커뮤니케이션 등 조직이해 및 관계형성 중심의 온보딩 교육</td>
								</tr>
								<tr>
									<td class="first">직급코스패스제</td>
									<td>· 해당 직급이 갖춰야할 필요역량을 바탕으로 4개 과정 구성, 순차적으로 수료하도록 코스화하여 운영하는 프로그램</td>
								</tr>
								<tr>
									<td class="first">사내 아카데미</td>
									<td>· 회사소개, 회사생활 가이드, 직무기초, 커뮤니케이션 등 조직이해 및 관계형성 중심의 온보딩 교육</td>
								</tr>
								<tr>
									<td class="first">직무기초교육</td>
									<td>· 마케팅, 영업, 세일즈, 스피치, 메뉴, 위생, CS 등 직무역량 개발 프로그램</td>
								</tr>
								<tr>
									<td class="first">
										B2B영업 실무자
										<br>양성과정
									</td>
									<td>· 신규고객 발굴 역량 강화를 위한 차별화된 마케팅 전략수립 및 제안 프레젠테이션 스킬 향상 과정</td>
								</tr>
								<tr>
									<td class="first">
										e커머스 실무자
										<br>양성과정
									</td>
									<td>· 자사몰 운영확대에 따른 e커머스 직무 전문성 강화 과정, GA(Google Analytics) 특강</td>
								</tr>
								<tr>
									<td class="first">
										DA(Data Analytics)
										<br>기초/심화 과정
									</td>
									<td>· 현업 실무자의 Data 이해 및 분석 능력 향상을 통한 데이터 기반 의사결정 역량 개발</td>
								</tr>
								<tr>
									<td class="first">사내 자격증반</td>
									<td>· 사내 전문가를 활용한 직무관련 자격증 취득반 운영 (바리스타, 와인 소믈리에, 건강 상담사 등)</td>
								</tr>
								<tr>
									<td class="first">사이버 대학 지원</td>
									<td>
										·임직원 대상 입학료/전형료 면제, 수업료 40~50% 감면 혜택
										<br>&nbsp;&nbsp;(고려대, 한양대, 경희대, 세종대, 숭실대, 한국외대, 서울사이버대)
									</td>
								</tr>
								<tr>
									<td class="first">마음챙김 프로그램</td>
									<td>· 임직원 스트레스 관리를 위한 팀단위 교육 프로그램 (MBTI, DISC, 컬러테라피, 아트테라피 등)</td>
								</tr>
								<tr>
									<td class="first">임직원 제휴혜택</td>
									<td>· 자기계발을 위한 외부업체 강의 20~30% 제휴혜택 제공 (밀리의 서재, 클래스101, 에듀윌, 해커스, 파고다 등)</td>
								</tr>
								<tr>
									<td class="first">온라인 교육 플랫폼</td>
									<td>· 직무, 리더십, 외국어, OA, 힐링, 소통 등 상시 열람 가능한 마이크로러닝 자료 제공</td>
								</tr>
								<tr>
									<td class="first">
										탄소중립 실현을 위한
										<br>기후환경교육
									</td>
									<td>
										· 전 사업부 대상 매년 기후환경교육을 실시하여 회사의 탄소중립 비전을 공유 (‘21.08)
										<br>· 교육 내용을 바탕으로 임직원이 회사의 정책에 참여할 수 있는 제도 마련
									</td>
								</tr>
							</tbody>
						</table>
					</div>
				</div>
			</section>
		</div>
	</div>
	<%@ include file="/Inc/Footer.jspf"%>
</body>

</html>